...
export function PromptInput2() {
  ...
  const placeholders = [
  "Design a fitness tracker mobile app.",
  "Create a food delivery app interface.",
  "Build a meditation app with timer features.",
  "Generate a mobile banking app layout.",
  "Design a social networking app for travelers.",
  "Create a task management app with reminders.",
  "Build a weather app with animated forecasts.",
  "Generate a language learning app interface.",
  "Design a music streaming app with playlists.",
  "Create a photo editing app with filters.",
];
  ...
  const handleSubmit = async () => {
    if (prompt.trim()) {
      try {
        ...
        setTimeout(() => {
          router.push(`/mobile/editor/?chat=${chatId}&prompt=${encodedPrompt}`)
        }, 1000);
      } catch (error:any) {
        setError(error.message)
        console.error('Error:', error)
  ...
  return (
    <div className="relative group">
      ...
      <button
        type="button"
        onClick={handleButtonClick}
        className="font-orbitron text-lg font-normal mt-4 w-full bg-[#8A2BE2] text-black hover:text-black py-2 rounded-lg hover:bg-[#6A1FA3] transition-colors"
      >
        Submit
      </button>
      <div className="w-full h-20 relative">
        {/* Fixed sparkle component that doesn't refresh on each keystroke */}
        <div className="absolute left-1/2 -translate-x-1/2 top-0 bg-gradient-to-r from-transparent via-[#6a0dad] to-transparent h-[3px] w-3/4 opacity-90" />
        <div className="absolute left-1/2 -translate-x-1/2 top-1 bg-gradient-to-r from-transparent via-[#5a189a]/70 to-transparent h-[1px] w-4/5 blur-[1px]" />
        <div className="absolute left-1/2 -translate-x-1/2 top-0.5 bg-gradient-to-r from-transparent via-[#4b0082]/50 to-transparent h-[5px] w-1/3 blur-[2px]" />
        <div className="absolute left-1/2 -translate-x-1/2 -top-1 bg-gradient-to-r from-transparent via-[#3a0ca3]/30 to-transparent h-[2px] w-full blur-sm" />
