interface ParsedProject {
  ...
  export function parseProjectString(input: string): ParsedProject {
    ...
    const fileSections = cleanedInput.split(/(?:\n|^)File: /g).filter(s => s.trim());
    ...
    for (const section of fileSections) {
      ...
      const fileName = firstNewline === -1 ? section.trim() : section.slice(0, firstNewline).trim();
      const content = firstNewline === -1 ? '' : section.slice(firstNewline + 1).trim();
